/*
In comparison to other versions. Here the explicit forth-order Ralstone method is applied
*/
#ifndef _SOLVER_
#define _SOLVER_
...
public:
		...
		// tforsin("for loop with sin"), tsinmemcpy("memcpy sin")
	{
		...
		checkHipError(hipMalloc(&dm_batched,
			SIZE * N * sizeof(float)));
        checkHipError(hipMalloc(&temp_dm, SIZE * N * sizeof(float)));
		checkHipError(hipMalloc(&k1, SIZE * N * sizeof(float)));
		checkHipError(hipMalloc(&k2, SIZE * N * sizeof(float)));
		checkHipError(hipMalloc(&k3, SIZE * N * sizeof(float)));
		checkHipError(hipMalloc(&k4, SIZE * N * sizeof(float)));
	...
	~Solver()
	{
		...
		checkHipError(hipFree(temp_dm));
		checkHipError(hipFree(k1));
		checkHipError(hipFree(k2));
		checkHipError(hipFree(k3));
		checkHipError(hipFree(k4));
    ...
    void compute_k3(float t, const ModelParams& pars)
    {
        ...
        setMd1(pars, t + coeff1);
        setMd2(pars, t + coeff1);
        checkRocblasError(rocblas_scopy(handle, N * SIZE, dm_batched, 1, temp_dm, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff2, k1, 1, temp_dm, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff3, k2, 1, temp_dm, 1));
        comp_M_dm_plus_B(temp_dm, k3);
    }
    
    void compute_k4(float t, const ModelParams& pars)
    {
        float coeff1 = delta_t * 0.21810040f;
        ...
        checkRocblasError(rocblas_scopy(handle, N * SIZE, dm_batched, 1, temp_dm, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff1, k1, 1, temp_dm, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff2, k2, 1, temp_dm, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff3, k3, 1, temp_dm, 1));
    ...
    void update_dm()
    {
        ...
        float coeff3 = delta_t * 1.20553560f;
        float coeff4 = delta_t * 0.17118478f;
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff1, k1, 1, dm_batched, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff2, k2, 1, dm_batched, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff3, k3, 1, dm_batched, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff4, k4, 1, dm_batched, 1));
    }

    void make_tmp_dm(const float* coeff, float* ki)
    {
        checkRocblasError(rocblas_scopy(handle, N * SIZE, dm_batched, 1, temp_dm, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, coeff, ki, 1, temp_dm, 1));
    }

    void comp_M_dm_plus_B(float* dm_, float* out_)
    {
        checkRocblasError(rocblas_sgemv_strided_batched(handle,rocblas_operation_transpose, N, N, &one, M_batched, N, totalN, dm_, 1, N, &zero, out_, 1, N, SIZE));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &one, B_batched, 1, out_, 1));
    }
	...
	void save_dm(vector<float>& dms)
	{
		...
		checkHipError(hipMemcpyDtoH(x.data(), dm_batched,
			x.size() * sizeof(float)));
