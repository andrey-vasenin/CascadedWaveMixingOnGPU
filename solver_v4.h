/*
In comparison to other versions. Here the explicit classical runge-kutta method is applied
*/
#ifndef _SOLVER_
#define _SOLVER_
...
public:
    ...
    {
        float coeff = delta_t/2.f;
        setM();
        setMd1(pars, t + coeff);
        setMd2(pars, t + coeff);
        make_tmp_dm(&coeff, k1);
    ...
    void compute_k4(float t, const ModelParams& pars)
    {
        ...
        make_tmp_dm(&delta_t, k3);
        comp_M_dm_plus_B(temp_dm, k4);
    }

    void update_dm()
    {
        float coeff1 = delta_t / 6.f;
        float coeff2 = delta_t / 3.f;
        ...
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff2, k3, 1, dm_batched, 1));
        checkRocblasError(rocblas_saxpy(handle, N * SIZE, &coeff1, k4, 1, dm_batched, 1));
    }
