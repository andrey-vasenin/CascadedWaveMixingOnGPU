...
void checkHipblasError(hipblasStatus_t&& stat)
{
#ifdef __DEBUG__
	if (stat != HIPBLAS_STATUS_SUCCESS)
	{
		cerr << rhipblasStatusToString(stat) << endl;
	}
#endif
}
...
public:
		...
		// tforsin("for loop with sin"), tsinmemcpy("memcpy sin")
	{
		...
		dm_host[0] = 1.f;
		log_vector("dm_host", dm_host);
		log_vector("ones_host", ones_host, 20);
		delta_t = dt;
		LEN = length;
		...
		checkHipError(hipMalloc(&dot_re_batched, SIZE * N * sizeof(float)));
		checkHipError(hipMalloc(&dot_im_batched, SIZE * N * sizeof(float)));
		...
		log("FFT arrays set");
		checkHipError(hipMemset(dm_fft_cos, 0, N * SIZE * ORDERS_NUM * sizeof(float)));
		checkHipError(hipMemset(dm_fft_sin, 0, N * SIZE * ORDERS_NUM * sizeof(float)));
		checkHipblasError(hipblasSetVector(N, sizeof(float),
			dm_host.data(), 1, dm, 1));
		checkHipblasError(hipblasSger(handle, N, SIZE, &one, dm, 1,
			ones, 1, dm_batched, N));
		setM0(pars);
	...
	~Solver()
	{
		...
		checkHipblasError(hiblasDestroy(handle));
	...
	void compute_next_step()
	{
		...
		checkRocblasError(rocblas_sgemv_strided_batched(handle, rocblas_operation_transpose, N, N, &delta_t, M_batched,
	...
	void add_to_fourier(float t)
	{
		...
		checkRocblasError(rocblas_sger(handle, SIZE * N, ORDERS_NUM,
			&delta_n, dm_batched, 1, cosines, 1, dm_fft_cos, SIZE * N));
		checkRocblasError(rocblas_sger(handle, SIZE * N, ORDERS_NUM,
			&delta_n, dm_batched, 1, sines, 1, dm_fft_sin, SIZE * N));
		// tsger.pause();
		// taddtofft.pause();
	}
